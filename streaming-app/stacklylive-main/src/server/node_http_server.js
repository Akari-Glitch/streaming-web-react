...
/* Adding User CHAT */
const { addUser, removeUser, getUser, getUsersInWorkshop } = require("../api/controllers/custom/users");
/* Adding User CHAT */
...
io.listen(8080);
    ...
    io.on("connect", (socket) => {
      try {
        socket.on("join", ({ name, workshop }, callback) => {
          ...
          socket.emit("message", {
            user: "St4cklyB0T",
            text: `Hey, ${user.name} bienvenido/a al taller! :)`,
          });
  ...
  run() {
    this.httpServer.listen(this.port, () => {
      Logger.log(`[StacklyLive] Http Server en el puerto: ${this.port}`);
    });

    this.httpServer.on('error', (e) => {
      Logger.error(`[StacklyLive] Http Server ${e}`);
    });

    this.httpServer.on('close', () => {
      Logger.log('[StacklyLive] Http Server Close.');
    });
    ...
    this.wsServer.on('listening', () => {
      Logger.log(`[StacklyLive] WebSocket Server started on port: ${this.port}`);
    });
    this.wsServer.on('error', (e) => {
      Logger.error(`[StacklyLive] WebSocket Server ${e}`);
    });

    if (this.httpsServer) {
      this.httpsServer.listen(this.sport, () => {
        Logger.log(`[StacklyLive] Https Server started on port: ${this.sport}`);
      });

      this.httpsServer.on('error', (e) => {
        Logger.error(`[StacklyLive] Https Server ${e}`);
      });

      this.httpsServer.on('close', () => {
        Logger.log('[StacklyLive] Https Server Close.');
      });
      ...
      this.wssServer.on('listening', () => {
        Logger.log(`[StacklyLive] WebSocketSecure Server started on port: ${this.sport}`);
      });
      this.wssServer.on('error', (e) => {
        Logger.error(`[StacklyLive] WebSocketSecure Server ${e}`);
      });
    }
