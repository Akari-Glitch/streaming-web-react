...
class NodeRtmpServer {
  constructor(config) {
    ...
    if (config.rtmp.ssl){
      ...
      try {
        ...
        Logger.error(`[StacklyLive] Rtmps Server error while reading ssl certs: <${e}>`);
      }
    }
  }
  ...
  run() {
    this.tcpServer.listen(this.port, () => {
      Logger.log(`[StacklyLive] Rtmp Server started on port: ${this.port}`);
    });

    this.tcpServer.on('error', (e) => {
      Logger.error(`[StacklyLive] Rtmp Server ${e}`);
    });

    this.tcpServer.on('close', () => {
      Logger.log(`[StacklyLive] Rtmp Server Close.`);
    });

    if (this.tlsServer) {
      this.tlsServer.listen(this.sslPort, () => {
        Logger.log(`[StacklyLive] Rtmps Server started on port: ${this.sslPort}`);
      });

      this.tlsServer.on('error', (e) => {
        Logger.error(`[StacklyLive] Rtmps Server ${e}`);
      });

      this.tlsServer.on('close', () => {
        Logger.log(`[StacklyLive] Rtmps Server Close.`);
      });
    }
  }
