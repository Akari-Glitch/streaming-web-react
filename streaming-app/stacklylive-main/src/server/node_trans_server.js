...
class NodeTransServer {
  ...
  async run() {
    try {
      ...
      Logger.error(`Trans Server startup failed. MediaRoot:${this.config.http.mediaroot} cannot be written.`);
      return;
    }
    ...
    try {
      ...
      Logger.error(`Trans Server startup failed. ffmpeg:${this.config.trans.ffmpeg} cannot be executed.`);
      return;
    }

    let version = await getFFmpegVersion(this.config.trans.ffmpeg);
    if (version === '' || parseInt(version.split('.')[0]) < 4) {
      Logger.error(`Trans Server startup failed. ffmpeg requires version 4.0.0 above`);
    ...
    Logger.log(`Trans Server started for apps: [ ${apps}] , MediaRoot: ${this.config.http.mediaroot}, ffmpeg version: ${version}`);
  }
